#!/usr/bin/env bash
# Build jetstream-indexer locally (linux/amd64 via buildx) and deploy to Cloud Run.
...
REGION=us-central1
SERVICE=jetstream-indexer
IMAGE="${REGION}-docker.pkg.dev/$PROJECT/jetstream-indexer/worker"
